...
export default function GroupsScreen() {
  ...
  return (
    <ScreenShell title="Nexus Hubs" subtitle="spatial groups · members orbit live">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 80, gap: 14 }}>
        {hubs.map((h) => (
          <Pressable
            key={h.id}
          ...
          >
            <Image source={NEBULAS[h.toneIndex]} style={StyleSheet.absoluteFill} contentFit="cover" />
            <LinearGradient
            ...
            <View style={styles.cardInner}>
              <View style={styles.cardTop}>
                {h.projectMode ? (
                  <View style={[styles.projectChip, { borderColor: colors.teal, backgroundColor: colors.teal + "22" }]}>
                    <Feather name="layers" size={10} color={colors.teal} />
                    <Text style={[styles.projectChipText, { color: colors.teal }]}>PROJECT MODE</Text>
                  </View>
                ) : <View />}
                <View style={styles.pulse}>
                  <View style={[styles.pulseDot, { backgroundColor: colors.gold }]} />
                  <Text style={styles.pulseText}>pulse {Math.round(h.pulse * 100)}</Text>
                </View>
              </View>
              ...
              <View style={styles.orbitRow}>
                {h.online.slice(0, 6).map((id, i) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={styles.orbitAvatarText}>{u?.avatarGlyph}</Text>
                    </View>
                  );
                })}
                <Text style={styles.orbitMeta}>
                  {h.online.length} orbiting · {(h.members / 1000).toFixed(1)}k members
                </Text>
              </View>
            </View>
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  orbitMeta: { color: "#A89AC8", fontFamily: "Inter_500Medium", fontSize: 11, marginLeft: 10 },
});
