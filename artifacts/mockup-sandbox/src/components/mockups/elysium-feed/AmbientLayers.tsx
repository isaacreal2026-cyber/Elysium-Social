...
const TIMELINE = [
  { id: 1, icon: "📝", author: "@stellarwave", content: "Patterns everywhere.", sparks: 3, time: "2m" },
  { id: 2, icon: "⚡", author: "@aurelia", content: "Three signs confirmed.", sparks: 5, time: "5m" },
  { id: 3, icon: "🎙", author: "@noxveil", content: "Ambient soundscape dropping soon", sparks: 2, time: "12m" },
  { id: 4, icon: "🔮", author: "@voidwalker", content: "Silence or Music? Vote.", sparks: 3, time: "18m" },
  { id: 5, icon: "❓", author: "@mirra", content: "Does time feel different to you?", sparks: 4, time: "25m" },
  { id: 6, icon: "📝", author: "@stellarwave", content: "Present. Grounded.", sparks: 2, time: "31m" },
];
    ...
    <div className="w-full max-w-md mx-auto h-[800px] flex flex-col bg-[#0a0514] text-slate-200 font-sans overflow-hidden relative selection:bg-indigo-500/30">
      ...
      <div className="pt-12 pb-3 px-6 z-10 flex flex-col gap-4 bg-gradient-to-b from-[#0a0514] to-transparent relative">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-medium tracking-wide text-indigo-100 flex items-center gap-2">
              <span className="text-indigo-400 font-light text-xl">∿</span> Ambient Stream
            </h1>
            <div className="flex items-end gap-[2px] h-3 ml-2 opacity-50">
              <div className="w-[2px] h-full bg-indigo-400" style={{ animation: "waveform-pulse 1.2s ease-in-out infinite" }}></div>
              <div className="w-[2px] h-[60%] bg-indigo-400" style={{ animation: "waveform-pulse 1.5s ease-in-out infinite 0.2s" }}></div>
              <div className="w-[2px] h-[80%] bg-indigo-400" style={{ animation: "waveform-pulse 1s ease-in-out infinite 0.4s" }}></div>
            </div>
          </div>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-1 text-sm font-medium">
          {ORBITS.map((orbit, i) => (
            <div key={orbit} className="relative whitespace-nowrap px-1 cursor-pointer group">
              <span className={`transition-colors ${i === 0 ? "text-indigo-200" : "text-slate-500 group-hover:text-slate-300"}`}>
                {orbit}
              </span>
              {i === 0 && (
          ...
          {STORIES.map((story) => {
            ...
            return (
              ...
              >
                {/* Aura Ring */}
                {isCenter && (
                  <div className={`absolute -inset-[3px] rounded-3xl ${story.ringClass} blur-[2px]`}></div>
                )}
                
                {/* Card Body */}
                <div className={`w-[240px] h-[280px] rounded-3xl ${story.bgClass} border border-white/5 flex flex-col justify-between p-6 relative overflow-hidden shadow-2xl shadow-black/50`}>
            ...
            >
              ...
              <div className="flex-1 flex flex-col ml-3 justify-center min-w-0">
                <div className="flex items-baseline gap-1.5 truncate">
                  <span className="text-[13px] font-medium text-indigo-200/80 shrink-0">{item.author}</span>
                  <span className="text-[13px] text-slate-400 truncate">· {item.content}</span>
                </div>
              </div>
