...
const TABS: { key: "moments" | "voice" | "hubs" | "saved"; label: string; icon: React.ComponentProps<typeof Feather>["name"] }[] = [
...
export default function ProfileScreen() {
  ...
  const { users, posts, hubs, isFollowing, toggleFollow, selfId, threads, bookmarks } = useResonance();
  const user = users.find((u) => u.id === id);
  const [tab, setTab] = useState<"moments" | "voice" | "hubs" | "saved">("moments");
  ...
  if (!user) {
    return (
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        <Text style={{ color: colors.text, padding: 24 }}>This soul has drifted away.</Text>
      </View>
    );
  }
  ...
  const showPosts =
    tab === "moments" ? userPosts : tab === "voice" ? voicePosts : tab === "saved" ? savedPosts : [];
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 130 }}>
        <View style={[styles.coverWrap, { paddingTop: topPad }]}>
          <View style={styles.cover}>
            <Image source={TONES[tone]} style={StyleSheet.absoluteFill} contentFit="cover" />
            <LinearGradient colors={["rgba(7,2,26,0.2)", "rgba(7,2,26,0.95)"]} style={StyleSheet.absoluteFill} />
          </View>
          <View style={styles.coverNav}>
            <Pressable onPress={() => router.back()} style={[styles.iconBtn, { backgroundColor: "rgba(7,2,26,0.6)" }]}>
              <Feather name="chevron-left" size={20} color="#fff" />
            </Pressable>
            <Pressable style={[styles.iconBtn, { backgroundColor: "rgba(7,2,26,0.6)" }]}>
        ...
        <View style={styles.profileBody}>
          <View style={[styles.avatar, { backgroundColor: user.avatarColor, borderColor: colors.background }]}>
            <Text style={styles.avatarText}>{user.avatarGlyph}</Text>
            {user.online ? <View style={[styles.onlineDot, { backgroundColor: colors.emerald, borderColor: colors.background }]} /> : null}
          </View>

          <View style={styles.nameRow}>
            <View style={{ flex: 1 }}>
              <Text style={[styles.name, { color: colors.text }]}>{user.name}</Text>
              <Text style={[styles.handle, { color: colors.mutedForeground }]}>{user.handle} · {user.city}</Text>
            </View>
            {!isMe ? (
          ...
          <View style={styles.voiceIntro}>
            ...
            <View style={styles.miniWave}>
              {Array.from({ length: 18 }).map((_, i) => (
                <View
                  key={i}
                  style={[styles.miniBar, { height: 4 + Math.abs(Math.sin(i * 0.6)) * 14, backgroundColor: colors.gold }]}
                />
              ))}
            </View>
            <Text style={[styles.voiceMeta, { color: colors.gold }]}>intro · {user.introVoiceSeconds}s</Text>
          </View>

          <View style={styles.statsRow}>
            <Stat label="followers" value={user.followers} />
            <Stat label="following" value={user.following} />
            <Stat label="alignment" value={`${Math.round(user.alignmentScore * 100)}`} />
            <Stat label="visitors / wk" value={user.weeklyVisitors} />
          </View>

          <View style={styles.tagRow}>
            {user.tags.map((t) => (
              <View key={t} style={[styles.tagChip, { borderColor: colors.border, backgroundColor: colors.primary + "11" }]}>
          ...
          <View style={styles.destRow}>
            {user.destinations.map((d) => (
              ...
              >
                ...
                <Text style={[styles.destChipText, { color: colors.gold }]}>{d}</Text>
              </Pressable>
            ))}
          </View>
        </View>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          {tab === "hubs" ? (
            userHubs.length === 0 ? (
              ...
              userHubs.map((h) => (
                <Pressable
                  ...
                  style={[styles.hubChip, { borderColor: colors.border, backgroundColor: colors.card }]}
                >
                  <View style={[styles.hubGlyph, { backgroundColor: colors.primary + "22" }]}>
                    <Feather name="hexagon" size={18} color={colors.primary} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.hName, { color: colors.text }]}>{h.name}</Text>
                    <Text style={[styles.hTag, { color: colors.mutedForeground }]} numberOfLines={1}>{h.tagline}</Text>
                  </View>
            ...
            <Empty text={tab === "saved" ? "nothing saved yet" : tab === "voice" ? "no voice notes yet" : "no moments yet"} />
          ) : (
...
const styles = StyleSheet.create({
  ...
  nameRow: { flexDirection: "row", alignItems: "center", marginTop: 12, gap: 12 },
  ...
  followBtn: { paddingHorizontal: 18, height: 38, borderRadius: 19, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  followText: { fontFamily: "Inter_700Bold", fontSize: 13 },
  bio: { fontFamily: "Inter_400Regular", fontSize: 14, lineHeight: 20, marginTop: 12 },
  voiceIntro: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  ...
  voicePlay: { width: 22, height: 22, borderRadius: 11, alignItems: "center", justifyContent: "center" },
  ...
  statLabel: { fontFamily: "Inter_500Medium", fontSize: 10, marginTop: 2, letterSpacing: 0.4, textTransform: "uppercase" },
  ...
  tabRow: { flexDirection: "row", marginTop: 18, marginHorizontal: 16, borderBottomWidth: 1 },
  tab: {
    flex: 1,
    alignItems: "center",
    justifyContent: "center",
  ...
  hubGlyph: { width: 40, height: 40, borderRadius: 12, alignItems: "center", justifyContent: "center" },
