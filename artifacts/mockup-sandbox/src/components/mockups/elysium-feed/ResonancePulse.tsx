...
import { Heart, RefreshCw, Layers, Sparkles, Filter, ChevronUp } from "lucide-react";
import "./_group.css";
...
const POSTS = [
  {
    ...
    type: "Destiny",
    content: "Following the pull of a new frequency. The stars align when you stop searching. ✦",
    time: "2h ago",
  ...
  },
  {
    ...
    type: "Voice",
    content: "late-night voice note dropped — my thoughts on the simulation hypothesis 🎙",
    time: "4h ago",
  ...
  },
  {
    ...
    type: "Note",
    content: "Sometimes the most cosmic thing you can do is be fully present on Earth.",
    time: "5h ago",
  ...
  },
  {
    ...
    type: "Project",
    content: "Building something wild. Phase 1 complete. Who wants early access?",
    time: "8h ago",
    ...
    <div className="w-full h-screen bg-[#05010a] overflow-hidden flex items-center justify-center relative font-sans">
      ...
      <div className="relative w-[390px] h-[844px] bg-black rounded-[3rem] border-[8px] border-zinc-900 shadow-2xl overflow-hidden z-10 flex flex-col">
        ...
        <div className="flex-1 relative w-full h-full">
           {/* Current Post */}
           <div className={`absolute inset-0 w-full h-full flex flex-col justify-between p-6 pt-16 pb-24 bg-gradient-to-b ${POSTS[activeIndex].color}`}>
              
              <div className="flex justify-between items-start">
                 <div className="flex items-center gap-3">
                   <Avatar className="w-10 h-10 border-2 border-white/10">
                     <AvatarFallback className="bg-zinc-800 text-white">{POSTS[activeIndex].author.avatar}</AvatarFallback>
                   </Avatar>
                   <div>
                     <div className="text-white font-medium text-sm">{POSTS[activeIndex].author.name}</div>
                     <div className="text-white/50 text-xs">@{POSTS[activeIndex].author.username} • {POSTS[activeIndex].time}</div>
                   </div>
                 </div>
                 <Badge variant="outline" className={`rounded-full px-3 py-1 text-xs ${POSTS[activeIndex].badgeColor}`}>
              ...
              <div className="glass-panel rounded-2xl p-4 flex justify-around items-center mb-8">
                 <button className="flex flex-col items-center gap-1 group">
                   ...
                   <span className="text-white/70 text-xs font-medium">{POSTS[activeIndex].metrics.spark}</span>
                 </button>
                 <button className="flex flex-col items-center gap-1 group">
                   <RefreshCw className="w-6 h-6 text-blue-400 group-hover:text-blue-300 transition-colors" />
                   <span className="text-white/70 text-xs font-medium">{POSTS[activeIndex].metrics.sync}</span>
                 </button>
                 <button className="flex flex-col items-center gap-1 group">
                   <Layers className="w-6 h-6 text-emerald-400 group-hover:text-emerald-300 transition-colors" />
                   <span className="text-white/70 text-xs font-medium">{POSTS[activeIndex].metrics.depth}</span>
                 </button>
              </div>
           ...
           {/* Next Post Peeking */}
           <div className={`absolute -bottom-[85%] left-0 w-full h-full bg-gradient-to-b ${POSTS[(activeIndex + 1) % POSTS.length].color} opacity-40 rounded-t-[3rem] blur-sm transition-transform`}></div>

           {/* Swipe Up Indicator */}
           <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center animate-bounce opacity-50">
              <ChevronUp className="w-6 h-6 text-white" />
              <span className="text-white text-[10px] tracking-widest uppercase">Pulse</span>
           </div>
